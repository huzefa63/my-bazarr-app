...
export async function addToCart({ id }) {
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/cart/add/${id}`, {
      method: "POST",
      headers: {
        Cookie: `token=${token}`,
      },
...
export async function checkoutAction(data) {
  ...
  const token = cookie.get("token")?.value;
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/cart/checkout`, {
      method: "POST",
      headers: {
        Cookie: `token=${token}`,
...
export async function getCartItems() {
  ...
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_URL}/cart/getCartItems`,
      {
        method: "GET",
        headers: {
